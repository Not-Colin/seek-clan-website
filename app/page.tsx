...
export default function Home() {
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      ...
      const [submissionsRes, activeBountiesRes] = await Promise.all([
      ...
      if (submissionsError || bountiesError) {
        ...
        if (allSubmissions) {
          ...
          approvedBounties.forEach(bounty => {
            bountyStats[bounty.bounty_tier!]++;
            bountyStats.total++;
            const rewardValue = bounty.bounty_tier === 'low' ? 2 : bounty.bounty_tier === 'medium' ? 5 : 10;
      ...
      // --- Fetch Google Sheets Data Separately ---
      const clanStatsApiUrl = process.env.NEXT_PUBLIC_CLAN_STATS_API_URL;
      if (clanStatsApiUrl) {
        try {
          const res = await fetch(clanStatsApiUrl);
          if (res.ok) {
            ...
            console.error("Failed to fetch clan stats: Response not OK");
          }
        } catch (error) {
          console.error("Error fetching clan stats from Google Sheets API:", error);
        }
      }
  ...
  return (
    // ... The entire JSX return block is unchanged from the last correct version.
      ...
      <main className="px-6 py-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <Link href="/bounties"><div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-xl p-6 hover:border-orange-500/50 transition-all duration-300 cursor-pointer"><div className="flex items-center justify-between mb-4"><div className="w-12 h-12 flex items-center justify-center bg-orange-600/20 rounded-lg"><i className="ri-sword-line text-2xl text-orange-400"></i></div></div><h3 className="text-gray-400 text-sm font-medium mb-2">Active Bounties</h3><p className="text-2xl font-bold text-white">{activeBounties}</p></div></Link>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-1 bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-xl p-6">
              ...
              {loading ? <p className='text-gray-400'>Loading...</p> : topEHB.length === 0 ? <p className="text-gray-400 text-center py-4">No EHB data found.</p> : (<div className="space-y-4">{topEHB.map((member, index) => (<div key={index} className="p-3 rounded-lg border bg-slate-700/30 border-slate-600/50 flex items-center justify-between"><span className="font-bold text-white">#{index + 1} {member.username}</span><span className="font-bold text-red-400">{member.ehb.toLocaleString()}</span></div>))}</div>)}
            </div>
            <div className="lg:col-span-1 bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-xl p-6">
              ...
              {loading ? <p className='text-gray-400'>Loading...</p> : topEHP.length === 0 ? <p className="text-gray-400 text-center py-4">No EHP data found.</p> : (<div className="space-y-4">{topEHP.map((member, index) => (<div key={index} className="p-3 rounded-lg border bg-slate-700/30 border-slate-600/50 flex items-center justify-between"><span className="font-bold text-white">#{index + 1} {member.username}</span><span className="font-bold text-blue-400">{member.ehp.toLocaleString()}</span></div>))}</div>)}
            </div>
            <div className="lg:col-span-1 bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-xl p-6">
