...
export default function EventsArchivePage() {
  ...
  const fetchAllEvents = useCallback(async () => {
    ...
    try {
      ...
      if (error) throw new Error('Failed to fetch event posts.');
      setPosts(data || []);
    } catch (err: any) {
      ...
      <main className="px-6 py-8">
        <div className="max-w-7xl mx-auto">
          <div className="mb-8 text-center">
            <h1 className="text-4xl font-bold text-white">Event Archive</h1>
            <p className="text-gray-400 text-lg mt-2">All past and present clan events.</p>
          </div>
          ...
          {!loading && !error && posts.length === 0 && (
              <p className="text-center text-gray-500">No event posts found.</p>
          )}
        </div>
      </main>
    </div>
