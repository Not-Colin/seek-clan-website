...
export default function AdminPage() {
    ...
    const fetchAllClanMembers = useCallback(async () => {
        const { data, error } = await supabase.from('player_details').select('wom_player_id, wom_details_json').order('wom_details_json->>displayName', { ascending: true });
        if (error) { console.error('Error fetching clan members:', error); } else { const members = data.map(p => ({ id: p.wom_player_id, displayName: p.wom_details_json?.displayName || 'Unknown' })).filter(p => p.displayName !== 'Unknown'); setAllClanMembers(members); if (members.length > 0) { setSelectedPlayerId(members[0].id.toString()); } }
    }, []);
    ...
    const checkUserAndLoadData = useCallback(async () => {
        ...
        setUser(currentUser);
        if (currentUser) {
            await Promise.all([fetchPendingSubmissions(), fetchBounties(), fetchPersonalBests(), fetchSettings(), fetchApprovedBounties(), fetchAllClanMembers()]);
        }
        setLoading(false);
    }, [fetchBounties, fetchPendingSubmissions, fetchPersonalBests, fetchSettings, fetchApprovedBounties, fetchAllClanMembers]);
    ...
    const handleBountyFileChange = useCallback((e: ChangeEvent<HTMLInputElement>) => {
        setNewBountyFile(null); if (e.target.files && e.target.files.length > 0) { const file = e.target.files[0]; const allowedTypes = ['image/png', 'image/jpeg', 'image/gif', 'image/webp']; if (!allowedTypes.includes(file.type)) { alert("Invalid file type."); e.target.value = ''; return; } setNewBountyFile(file); }
    }, []);
