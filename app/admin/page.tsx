// app/admin/page.tsx - FINAL with Trade Log functionality fully integrated
...
// Interfaces
interface Submission { id: number; created_at: string; player_name: string; submission_type: string; bounty_name: string | null; personal_best_category: string | null; proof_image_url: string; is_archived: boolean; status: string; bounty_tier: 'low' | 'medium' | 'high' | null; personal_best_time: string | null; trade_proof_url: string | null; }
interface Bounty { id: number; name: string; tier: string; image_url: string; is_active: boolean; created_at: string; }
// Updated this interface to include the trade_proof_url for conditional rendering
interface ApprovedBounty { id: number; player_name: string; bounty_name: string | null; trade_proof_url: string | null; }
...
export default function AdminPage() {
    ...
    const fetchApprovedBounties = useCallback(async () => {
        const { data, error } = await supabase.from('submissions').select('*').eq('status', 'approved').eq('submission_type', 'bounty').order('created_at', { ascending: false });
    ...
    const handleUpdateStatus = useCallback(async (submissionToUpdate: Submission, newStatus: 'approved' | 'rejected') => {
        ...
        if (newStatus === 'approved' && submissionToUpdate.submission_type === 'bounty' && submissionToUpdate.bounty_name) {
            const { error: bountyUpdateError } = await supabase.from('bounties').update({ is_active: false }).eq('name', submissionToUpdate.bounty_name);
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
            ...
            <main className="px-6 py-8">
                <div className="max-w-7xl mx-auto">
                    ...
                    {activeTab === 'validate' && (
                        <div className="bg-slate-800/50 rounded-xl p-6"><h3 className="text-lg font-semibold text-white mb-4">Pending Reviews</h3>{pendingSubmissions.length === 0 ? <p className="text-gray-400">No pending submissions.</p> : (<div className="space-y-4">{pendingSubmissions.map((sub) => (<div key={sub.id} className="bg-slate-700/50 p-4 rounded-lg flex items-center justify-between flex-wrap gap-4"><div><p className="text-white font-bold">{sub.player_name}</p><p className="text-gray-300 text-sm">{sub.submission_type === 'bounty' ? `Bounty: ${sub.bounty_name}` : `PB: ${sub.personal_best_category}`}</p></div><div className="flex items-center space-x-4"><a href={sub.proof_image_url} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300 text-sm">View Proof</a><button onClick={() => handleUpdateStatus(sub, 'approved')} className="bg-green-600 hover:bg-green-700 text-white px-3 py-1 rounded-md text-sm">Approve</button><button onClick={() => handleUpdateStatus(sub, 'rejected')} className="bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded-md text-sm">Reject</button></div></div>))}</div>)}</div>
                    )}
                    {activeTab === 'pbs' && (
                    ...
                    {activeTab === 'tradelog' && (
                        <div className="bg-slate-800/50 rounded-xl p-6"><h3 className="text-lg font-semibold text-white mb-4">Manage Bounty Payouts</h3><p className="text-sm text-gray-400 mb-6">A list of all approved bounties. Log, update, or remove trade proof screenshots.</p>{approvedBounties.length === 0 ? <p className="text-gray-400">No approved bounties found.</p> : (<div className="space-y-6">{approvedBounties.map((submission) => (<div key={submission.id} className="bg-slate-700/50 p-4 rounded-lg"><p className="text-white font-bold">{submission.player_name}</p><p className="text-gray-300 text-sm">Bounty: {submission.bounty_name}</p>{submission.trade_proof_url ? (<div className="mt-4 flex flex-col sm:flex-row items-center gap-4"><a href={submission.trade_proof_url} target="_blank" rel="noopener noreferrer" className="flex-grow w-full text-center sm:text-left text-blue-400 hover:underline">View Logged Trade Proof</a><button onClick={() => handleTradeProofRemove(submission)} disabled={isLoggingTrade && selectedSubmissionId === submission.id} className="w-full sm:w-auto bg-red-600 hover:bg-red-700 text-white font-medium py-2 px-4 rounded-lg disabled:bg-slate-600">{isLoggingTrade && selectedSubmissionId === submission.id ? 'Removing...' : 'Remove Proof'}</button></div>) : (<form onSubmit={(e) => handleTradeLogSubmit(e, submission.id)} className="mt-4 flex flex-col sm:flex-row items-center gap-4"><input type="file" onChange={(e) => { setSelectedSubmissionId(submission.id); setTradeProofFile(e.target.files ? e.target.files[0] : null); }} required accept="image/*" className="flex-grow w-full text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:font-semibold file:bg-blue-600/20 file:text-blue-300"/><button type="submit" disabled={isLoggingTrade && selectedSubmissionId === submission.id} className="w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-4 rounded-lg disabled:bg-slate-600 whitespace-nowrap">{isLoggingTrade && selectedSubmissionId === submission.id ? 'Logging...' : 'Log Trade'}</button></form>)}</div>))}</div>)}</div>
                    )}
                    {activeTab === 'settings' && (
