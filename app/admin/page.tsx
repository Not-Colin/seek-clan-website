...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState('post'); // Default tab is now 'post'
  ...
  const handleUpdateStatus = async (id: number, status: 'approved' | 'rejected') => {
    ...
    if (error) alert(`Error: ${error.message}`);
      ...
      fetchPendingSubmissions();
      // If a PB was approved, we need to refresh that list too for the other tab
      if (status === 'approved') {
        fetchPersonalBests();
      }
    }
  ...
  const handleBountyFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) setNewBountyFile(e.target.files[0]);
  };
  ...
  const handleBountyArchive = async (id: number, currentStatus: boolean) => {
    const { error } = await supabase.from('bounties').update({ is_active: !currentStatus }).eq('id', id);
    if (error) alert(`Error: ${error.message}`);
    else fetchBounties();
  };

  const handlePbArchive = async (id: number, currentStatus: boolean) => {
      ...
      <main className="px-6 py-8">
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={handleLogout} className="bg-red-600 hover:bg-red-700 text-white font-medium py-2 px-4 rounded-lg">Logout</button>
          </div>

          {/* --- Tab Navigation (Reordered and Renamed) --- */}
          <div className="flex border-b border-slate-700 mb-6">
            <button onClick={() => setActiveTab('post')} className={`px-4 py-3 text-sm font-medium ${activeTab === 'post' ? 'text-orange-400 border-b-2 border-orange-400' : 'text-gray-400'}`}>
              Post Bounties
            </button>
            <button onClick={() => setActiveTab('validate')} className={`px-4 py-3 text-sm font-medium ${activeTab === 'validate' ? 'text-orange-400 border-b-2 border-orange-400' : 'text-gray-400'}`}>
              Validate Bounties
            </button>
            <button onClick={() => setActiveTab('pbs')} className={`px-4 py-3 text-sm font-medium ${activeTab === 'pbs' ? 'text-orange-400 border-b-2 border-orange-400' : 'text-gray-400'}`}>
              Manage Personal Bests
            </button>
          </div>
              ...
              <div className="md:col-span-2">
                <div className="bg-slate-800/50 rounded-xl p-6">
                  ...
                  <div className="space-y-3">
                    {bounties.map(bounty => (
                      <div key={bounty.id} className="bg-slate-700/50 p-3 rounded-lg flex items-center justify-between">
                        <div><p className={`font-bold ${bounty.is_active ? 'text-white' : 'text-gray-500 line-through'}`}>{bounty.name}</p><p className="text-sm text-gray-400 capitalize">{bounty.tier} Tier</p></div>
                        <button onClick={() => handleBountyArchive(bounty.id, bounty.is_active)} className={`px-3 py-1 text-sm rounded-md ${bounty.is_active ? 'bg-red-600 hover:bg-red-700' : 'bg-green-600 hover:bg-green-700'}`}>{bounty.is_active ? 'Archive' : 'Re-activate'}</button>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {activeTab === 'validate' && (
            <div className="bg-slate-800/50 rounded-xl p-6">
              ...
              {pendingSubmissions.length === 0 ? <p className="text-gray-400">No pending submissions.</p> : (
                <div className="space-y-4">{pendingSubmissions.map((sub) => (
                  <div key={sub.id} className="bg-slate-700/50 p-4 rounded-lg flex items-center justify-between flex-wrap gap-4">
                    <div><p className="text-white font-bold">{sub.player_name}</p><p className="text-gray-300 text-sm">{sub.submission_type === 'bounty' ? `Bounty: ${sub.bounty_name}` : `PB: ${sub.personal_best_category}`}</p></div>
                    <div className="flex items-center space-x-4">
                       ...
                       <button onClick={() => handleUpdateStatus(sub.id, 'approved')} className="bg-green-600 hover:bg-green-700 text-white px-3 py-1 rounded-md text-sm">Approve</button>
                       <button onClick={() => handleUpdateStatus(sub.id, 'rejected')} className="bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded-md text-sm">Reject</button>
                    </div>
                  </div>))}
                </div>
              )}
          ...
          {activeTab === 'pbs' && (
            <div className="bg-slate-800/50 rounded-xl p-6">
              ...
              <div className="space-y-3">
                {personalBests.length === 0 ? <p className="text-gray-400">No approved personal bests yet.</p> :
                  personalBests.map(pb => (
                    <div key={pb.id} className="bg-slate-700/50 p-3 rounded-lg flex items-center justify-between">
                      <div>
                        <p className={`font-bold ${!pb.is_archived ? 'text-white' : 'text-gray-500 line-through'}`}>{pb.player_name} - {pb.personal_best_category}</p>
                        <p className="text-sm text-orange-400">Time: {pb.personal_best_time}</p>
                      </div>
                      <button onClick={() => handlePbArchive(pb.id, pb.is_archived)} className={`px-3 py-1 text-sm rounded-md ${!pb.is_archived ? 'bg-red-600 hover:bg-red-700' : 'bg-green-600 hover:bg-green-700'}`}>
                        {!pb.is_archived ? 'Archive' : 'Re-activate'}
                      </button>
                    </div>
                  ))
                }
